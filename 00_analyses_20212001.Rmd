...
source("R/01_plot_theme_211112.R")
...
> Code-Author: Gian-Andrea Egeler [based on 2020](https://github.com/GAEgeler/tilldata_2017)
...
df_nova <- nova_ind %>% 
  ...
  ungroup() %>% 
  mutate(label_content = if_else(is.na(.$label_content), "Unknown", 
                                 .$label_content),
           gender = "All") %>% 
  mutate(label_content = recode(label_content, 
                                ...
                                "Fisch"="Fish",
                                "Fleisch" = "Meat")) %>% 
  mutate(condit = if_else(condit == "Basis", "Base week", "Intervention"))
...
df_gender %<>% 
    ...
    mutate(text = paste("n = ", format(tot_sold, big.mark = "'"), sep = "")) %>% 
    select(-tot_sold) %>% 
    left_join(df_gender, .[ , c("gender", "condit", "text")],
              by = c("gender", "condit")) %>% 
    mutate(xlab = paste(condit, text, sep = "\n"))
...
p <- ggplot(dat, 
        ...
        geom_text(aes(label=ifelse(pct < 0.02,"", 
                  ...
                  fontface = "bold") + 
        coord_flip() +
        facet_wrap(~factor(gender, levels = c("All", "Female", "Male")), 
                   nrow = NULL, 
                   ncol = 1, 
                   scales = "free_y") +
        mytheme_survey 
...
df_nova <- nova_ind %>% 
  ...
  mutate(label_content = recode(label_content, 
                                ...
                                "Fleisch" = "Meat")) %>%
  mutate(gender = ifelse(.$gender == "F", "Female", "Male")) %>% 
  mutate(member = if_else(member == "Studierende", "Students", "Staff"))
...
df_nova <- nova_ind %>% 
  ...
  mutate(age_group = recode(age_group, 
                            "15 bis 25-jährig" = "15 to 25 years old",
                            "26 bis 34-jährig" = "26 to 34 years old",
                            "35 bis 49-jährig" = "35 to 49 years old",
                            "50 bis 64-jährig" = "50 to 64 years old"
                            )) %>%
  mutate(gender = ifelse(.$gender == "F", "Female", "Male")) 
...
df_nova <- nova_ind %>% 
  ...
  summarise(tot_sold = n()) %>% 
  ungroup() %>% 
  mutate(label_content = if_else(is.na(.$label_content), "Unbekannt", 
                                 .$label_content)) %>% 
  mutate(label_content = recode(label_content, 
                                "Unbekannt" = "Unknown",
                                "Pflanzlich" = "Vegan (meat substitute)", 
...
dat <- df_text %>% 
    ...
    mutate(text = paste("n = ", format(tot_sold, big.mark = "'"), sep = "")) %>% 
    select(-tot_sold) %>% 
    left_join(df_text, .[ , c("gender", "member", "text")], 
              by = c("gender", "member")) %>% 
...
dat <- df_text %>% 
    ...
    mutate(text = paste("n = ", format(tot_sold, big.mark = "'"), sep = "")) %>% 
    select(-tot_sold) %>% 
    left_join(df_text, .[ , c("age_group", "gender", "text")], 
...
p <- ggplot(dat, aes(y = pct_member, 
                    ...
                    fill = factor(lunch_type, 
                                  levels = NamesPerType$lunch_type), 
                    color = label_color)) + 
  geom_bar(stat = "identity", 
  ...
  scale_fill_manual(values = ColsPerCat_blind, 
                      ...
                      labels = NamesPerType$plot_names_rmp) +
  geom_text(aes(label=ifelse(pct_member<0.02,"", 
                               scales::percent(round(pct_member, 2),
                                               accuracy = 1))),
            size = 22 * converter, 
...
ggsave(filename = paste0(wd_plots, "/pos_lunch_type_affiliation_",
                         format(Sys.Date(), "%y%m%d"),
                         ".pdf"),
       p,
       height = 5,
...
df_mensa_satis1 <- df_satis %>% 
  ...
  select(-c("satis_1")) %>% 
  mutate(question = "My expectations from the meal choice were met.")
...
df_mensa_satis2 <- df_satis %>% 
  ...
  select(-c("satis_2")) %>% 
  mutate(question = "The meal was good.")
...
df_mensa_satis3 <- df_satis %>% 
  ...
  mutate(Wahl=satis_3) %>% 
  select(-c("satis_3")) %>% 
  mutate(question = "I would not choose this meal again.")
...
## plot ----
p <- ggplot(data = dat, 
       aes(x = forcats::fct_rev(intervention), 
           y = pct_answ, 
  ...
  ggtitle("How satisfied are you with your meal choice?
\n(Question 3, Experiment week, n = 874)")+
  coord_flip() +
  mytheme_survey +
  facet_wrap(~question,  nrow = NULL, ncol = 1, scales = "free_y") +
  scale_y_origin(labels = scales::percent)
  
ggsave(filename = paste0(wd_plots, "/survey_choice_satisfaction_",
                         format(Sys.Date(), "%y%m%d"),
                         ".pdf"),
       p,
       width = 23,
...
df_meat <- nova_survey %>%
  filter(fill == 0) %>% 
  filter(!is.na(meat)) %>% 
  mutate(meat_diet = case_when (meat==7 ~ "Meat lover (dmp) (n = 146)",
                              meat==6 ~ "Meat lover (dmp) (n = 146)",
                              meat==5 ~ "Meat eater (dmp) (n = 118)",
                              meat==4 ~ "Meat-flexitarian (dmp) (n = 177)",
                              meat==3 ~ "Vegetarian-flexitarian (dmp) (n = 183)",
                              meat==2 ~ "Meat avoider (dmp) (n = 139)",
                              meat==1 ~ "Meat avoider (dmp) (n = 139)", 
...
p <- ggplot(data = dat, 
  ...
  scale_x_discrete(limits=c("Meat avoider (dmp) (n = 139)",
                            "Vegetarian-flexitarian (dmp) (n = 183)",
                            "Meat-flexitarian (dmp) (n = 177)",
                            "Meat eater (dmp) (n = 118)",
                            "Meat lover (dmp) (n = 146)"))+
  ...
  ylab("")+
  guides(color = "none", fill = guide_legend("", reverse = TRUE, nrow=1)) +
  ggtitle("In general, to what extent do you agree with the following statements on eating habits?\n(Question 7, Type of meat preference, n = 763)\n\nIn general, I think about the consequences...")+
  coord_flip() +
  mytheme_survey +
  facet_wrap(~question,  nrow = NULL, ncol = 1, scales = "free_y") +
  scale_y_origin(labels = scales::percent)
...
ggsave(filename = paste0(wd_plots, "/survey_lunch_type_attitude_",
                         format(Sys.Date(), "%y%m%d"),
                         ".pdf"),
       p, 
       width = 26,
...
df_gender <- nova_survey %>% 
  ...
  mutate(gender_2 = if_else(gender=="Mann", "Male (n = 651)", "Female (n = 495)"))
...
df_gender_tra1 <- df_gender %>% 
  ...
  mutate(order_plot = 2) %>% 
  select(-c("tra_1")) %>% 
  mutate(question = "...to live a healthy lifestyle.")
...
df_gender_tra2 <- df_gender %>% 
  ...
  mutate(order_plot = 4)  %>% 
  select(-c("tra_2")) %>% 
  mutate(question = "...to pollute the environment as little as possible through my own actions.")
...
df_gender_tra3 <- df_gender %>% 
  ...
  mutate(order_plot = 5)  %>% 
  select(-c("tra_3")) %>% 
  mutate(question = "...that the world become a socially more just place.")
...
df_gender_tra4 <- df_gender %>% 
  ...
  mutate(order_plot = 3)  %>% 
  select(-c("tra_4")) %>% 
  mutate(question = "...that working conditions are humane for everyone.")
...
df_gender_tra5 <- df_gender %>% 
  ...
  mutate(order_plot = 1)  %>% 
  select(-c("tra_5")) %>% 
  mutate(question = "...that, as far as possible, livestock is kept in species-appropriate conditions.")
...
p <- ggplot(data = dat,
  ...
  ylab("")+
  guides(color = "none", fill = guide_legend("", reverse = TRUE, nrow = 1)) +
  ggtitle("In general, to what extent do you agree with the following statements?\n(Question 8, Gender, n = 1,146)\n\nIn general, I think it is important...")+
  coord_flip() +
  mytheme_survey +
  facet_wrap(~question,  nrow = NULL, ncol = 1, scales = "free_y") +
  scale_y_origin(labels = scales::percent)
...
ggsave(filename = paste0(wd_plots, "/survey_gender_thoughts_",
                         format(Sys.Date(), "%y%m%d"),
                         ".pdf"),
       p,
       width = 22,
